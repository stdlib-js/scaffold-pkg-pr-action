...
tape( 'the command-line interface tests if an input argument is a domain name', opts, function test( t ) {
	var cmd = [
		EXEC_PATH,
		'-e',
...
tape( 'when used as a standard stream, if provided a delimiter option which is not a regular expression, the command-line interface prints an error and sets a non-zero exit code', opts, function test( t ) {
	var script;
	var opts;
	var cmd;

	script = readFileSync( resolve( __dirname, 'fixtures', 'bad_split.js.txt' ), {
		'encoding': 'utf8'
	});
	...
	function done( error, stdout, stderr ) {
		...
		t.strictEqual( stdout.toString(), '', 'does not print to `stdout`' );
		t.strictEqual( stderr.toString(), 'TypeError: split option must be a regular expression\n', 'expected value' );
		t.end();
	}
});
